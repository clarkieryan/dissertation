...
describe Facebook do
 	...
 	before :each do
 		stub_request(:get, "https://graph.facebook.com/oauth/access_token?client_id=app_id&client_secret=app_secret&grant_type=client_credentials&redirect_uri=http://localhost").to_return(:body => "access_token=accesstoken");
	...
	it "has a constant of the base url " do
		@facebook.getBaseUri.should  eq("https://graph.facebook.com")
	end
	...
	it "#getOAuthToken" do
		stub_request(:get, "https://graph.facebook.com/oauth/access_token").to_return(:body => "access_token=access|token");
